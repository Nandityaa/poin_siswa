...
// Data Orang Tua / Wali (dikirim dari file add_perjanjian_ortu.php)
...
$tanggal_lahir_formatted = tanggal_indonesia($tanggal_lahir);
...
$cek_surat = mysqli_query($conn, "SELECT id_surat_keluar FROM surat_keluar WHERE jenis_surat = 'perjanjian_ortu' AND nis = '$nis' AND tanggal_pembuatan_surat = '$tgl_sekarang'");
if(mysqli_num_rows($cek_surat) == 0) {
    mysqli_query($conn, "INSERT INTO surat_keluar (id_tingkat, jenis_surat, nis, tanggal_pembuatan_surat) VALUES ('$id_tingkat', 'perjanjian_ortu', '$nis', '$tgl_sekarang')");
}

// Insert ke perjanjian_orang_tua
$cek_perjanjian = mysqli_query($conn, "
    SELECT po.id_perjanjian_ortu 
    FROM perjanjian_orang_tua po 
    JOIN pelanggaran_siswa pel ON po.id_pelanggaran_siswa = pel.id_pelanggaran_siswa 
    WHERE pel.nis = '$nis' AND DATE(po.tanggal) = '$tgl_sekarang'
");

if(mysqli_num_rows($cek_perjanjian) == 0) {
    // Get latest pelanggaran for this student
    $q_pelanggaran = mysqli_query($conn, "SELECT id_pelanggaran_siswa FROM pelanggaran_siswa WHERE nis = '$nis' ORDER BY tanggal DESC LIMIT 1");
    if($r_pelanggaran = mysqli_fetch_assoc($q_pelanggaran)) {
        $id_pelanggaran = $r_pelanggaran['id_pelanggaran_siswa'];
        $waktu_sekarang = date('Y-m-d H:i:s');
        $id_ortu = $row_siswa['id_ortu_wali'] ?? NULL;
        if ($id_ortu) {
            mysqli_query($conn, "INSERT INTO perjanjian_orang_tua (tanggal, id_pelanggaran_siswa, id_ortu_wali, status, tingkat) VALUES ('$waktu_sekarang', '$id_pelanggaran', '$id_ortu', 'Masih Proses', '$tingkat')");
        } else {
            echo "<script>alert('Data orang tua belum lengkap di database siswa. Tidak dapat membuat surat perjanjian.'); window.location.href='cetak.php';</script>";
            exit;
        }
    } else {
        echo "<script>alert('Siswa ini belum memiliki data pelanggaran. Tidak dapat membuat surat perjanjian.'); window.location.href='cetak.php';</script>";
        exit;
    }
}
    ...
    <div class="content">
        ...
        <div class="indent">
            ...
            <div class="form-row">
                ...
                <div class="separator">:</div>
                <div class="field"><?= htmlspecialchars($tempat_lahir) ?>, <?= $tanggal_lahir_formatted ?></div>
            </div>
            <div class="form-row">
        ...
        <p>Adalah orang tua/wali dari siswa :</p>
        ...
        <p class="statement">
            Menyatakan dan berjanji akan ikut bertanggung jawab dalam membina dan mengawasi anak saya selama menempuh pendidikan di SMK TI Bali Global Denpasar.
            Apabila anak saya melakukan pelanggaran dan tidak ada perubahan, maka saya menerima segala keputusan dari pihak sekolah.
        </p>
        <p class="statement">
