...
$cek_surat = mysqli_query($conn, "SELECT id_surat_keluar FROM surat_keluar WHERE jenis_surat = 'perjanjian_siswa' AND nis = '$nis' AND tanggal_pembuatan_surat = '$tgl_sekarang'");
if(mysqli_num_rows($cek_surat) == 0) {
    mysqli_query($conn, "INSERT INTO surat_keluar (id_tingkat, jenis_surat, nis, tanggal_pembuatan_surat) VALUES ('$id_tingkat', 'perjanjian_siswa', '$nis', '$tgl_sekarang')");
}

// Insert ke perjanjian_siswa
$cek_perjanjian = mysqli_query($conn, "
    SELECT ps.id_perjanjian_siswa 
    FROM perjanjian_siswa ps 
    JOIN pelanggaran_siswa pel ON ps.id_pelanggaran_siswa = pel.id_pelanggaran_siswa 
    WHERE pel.nis = '$nis' AND DATE(ps.tanggal) = '$tgl_sekarang'
");
...
if(mysqli_num_rows($cek_perjanjian) == 0) {
    ...
    if($r_pelanggaran = mysqli_fetch_assoc($q_pelanggaran)) {
        ...
        mysqli_query($conn, "INSERT INTO perjanjian_siswa (tanggal, id_pelanggaran_siswa, status, tingkat) VALUES ('$waktu_sekarang', '$id_pelanggaran', 'Masih Proses', '$tingkat')");
    } else {
